...
import { passageScan } from '../../services/checkpoints.ts';
...
import { actorFrom, authorized, rateLimited } from '../permission.ts';
...
export const checkinRouter = router({
  /** Validate a code/QR and return visit details for confirmation (public, rate-limited). */
  lookup: rateLimited(20, 60)
  ...
  /** Self-service check-in at a kiosk (public, rate-limited). */
  complete: rateLimited(20, 60)
    .input(checkInSubmitSchema)
    .mutation(({ input, ctx }) =>
      completeCheckIn(input.lookup, {
        ip: ctx.ip,
        facilityId: input.facilityId,
        deviceId: input.deviceId,
      }),
  ...
  /** Self-service check-out by invitation code or badge QR (public, rate-limited). */
  checkoutSelf: rateLimited(20, 60)
    .input(
      z.object({
    ...
    .mutation(({ input, ctx }) =>
      checkOutSelf(input.lookup, { ip: ctx.ip, deviceId: input.deviceId, facilityId: input.facilityId }),
    ),
  ...
  /** Internal-checkpoint passage scan — logs a checked-in visitor passing a checkpoint (no state change). */
  ...
  /** Return a tag — frees it for reuse and checks the visitor out (public kiosk, rate-limited). */
  returnTag: rateLimited(30, 60)
    .input(tagReturnSchema)
    .mutation(({ input, ctx }) => returnTag(input, { ip: ctx.ip })),
  ...
  welcome: rateLimited(60, 60).query(async () => {
    ...
    if (!welcomeCache.has(key)) {
      const { audio, mimeType } = await synthesize(WELCOME_TEXT[language], { language, voice, speed });
