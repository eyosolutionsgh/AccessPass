...
reads that profile and decides *which* path to use; this shell *provides* the native paths it asks
...
`"lockTask": true` calls `startLockTask()`. To pin **without** the system confirmation dialog, make
the app a device owner (factory-fresh / no accounts) and allow-list it:
...
src/main/
  ...
  java/com/vms/kiosk/
    MainActivity.java            WebView + JS bridge + lifecycle + lock task
